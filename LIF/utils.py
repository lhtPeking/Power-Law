...
class LIFNetwork_GaussianWeight:
    def __init__(
        self,
        ...
        w_scale=0.1e-9,  # float (A): standard deviation (scale) of synaptic weights
        I_ext=1.5e-9,    # float (A): constant external input current to each neuron
        seed=None,       # int or None: random seed for reproducibility
        record_spikes=True,  # bool: whether to store full spike raster (steps x N)
        record_voltage=True
    ):
        """
        Simple current-based LIF network with exponential synaptic currents.

        All units are in SI:
            time in seconds, voltage in volts, resistance in ohms, current in amperes.
        """
        if seed is not None:
            np.random.seed(seed)
        ...
        # ===== 4. External input =====
        self.I_ext = I_ext                                      # external constant current (A)
    ...
    def step(self):
        ...
        # Total input current = synaptic current + external constant current
        I_total = self.I_syn + self.I_ext_vec
...
class LIFNetwork_PowerLawWeight:
    def __init__(
        self,
        N=1000,          # int: number of neurons in the network
        T=1.0,           # float (s): total simulation time
